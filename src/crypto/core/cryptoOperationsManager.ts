import CryptoOperations from "../services/cryptoOperations";
import EnvironmentFileManager from "../managers/environmentFileManager";
...
export default class CryptoOperationsManager {
  private environmentFileManager: EnvironmentFileManager;

  constructor() {
    this.environmentFileManager = new EnvironmentFileManager();
  }
  ...
  public async encryptEnvVariables(envVariables?: string[]): Promise<void> {
    try {
      ...
      const variablesToEncrypt = this.determineVariablesToEncrypt(
        allEnvVariables,
        envVariables
      );

      // Update lines with encrypted values
      const { updatedLines, encryptedCount } =
        await this.processVariablesForEncryption(
          envFileLines,
          variablesToEncrypt
        );
  ...
  private determineVariablesToEncrypt(
    ...
    if (envVariables && envVariables.length > 0) {
      for (const lookupValue of envVariables) {
        const found = this.findEnvironmentVariable(
          allEnvVariables,
          lookupValue
        );
        if (!found) {
  ...
  private async encryptIfNeeded(value: string, secretKey: string) {
    ...
    // Encrypt the value
    const encryptedResult = await CryptoOperations.encrypt(value, secretKey);
...
private logEncryptionSuccess(
   ...
   */
  public async storeSecretKeyInEnvironmentFile(
    keyName: string,
    keyValue: string
  ) {
    try {
