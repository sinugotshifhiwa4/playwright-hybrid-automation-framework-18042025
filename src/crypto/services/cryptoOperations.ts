...
import { CRYPTO_CONFIG } from "../parameters/cryptoParameters";
import { EncryptionParameters } from "../parameters/encryptionParameters";
import { FileEncoding } from "../../config/constants/fileEncodingParams";
...
export default class CryptoOperations {
  ...
  private static readonly MEMORY_COST =
    CRYPTO_CONFIG.ARGON2_PARAMETERS.MEMORY_COST;
  private static readonly TIME_COST = CRYPTO_CONFIG.ARGON2_PARAMETERS.TIME_COST;
  private static readonly PARALLELISM =
    CRYPTO_CONFIG.ARGON2_PARAMETERS.PARALLELISM;
  ...
  public static async decryptMultipleKeys(
    ...
    try {
      ...
      return await Promise.all(
        encryptedDataArray.map((data) => this.decrypt(data, secretKey))
      );
    } catch (error) {
      ErrorHandler.captureError(
        error,
   ...
   */
  private static async deriveKeyWithArgon2(
    secretKey: string,
    salt: string
  ): Promise<CryptoKey> {
    try {
   ...
   */
  private static async importKeyForCrypto(
    keyBuffer: string
  ): Promise<CryptoKey> {
    try {
