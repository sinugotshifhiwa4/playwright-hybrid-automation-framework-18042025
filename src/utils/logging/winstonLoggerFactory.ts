...
import fs from "fs";
import { WINSTON_LOGGER_PARAMS } from "../../models/interfaces/winstonLoggerParams";
  ...
  public static customTimestampFormat() {
    try {
      ...
      const timestampFormat = winston.format.timestamp({
        format: () =>
          moment()
            .tz(WINSTON_LOGGER_PARAMS.TIME_ZONE)
            .format(WINSTON_LOGGER_PARAMS.DATE_FORMAT),
      });
  ...
  public static createConsoleTransport(
    ...
    try {
      ...
      const consoleTransport = new winston.transports.Console({
        level: WINSTON_LOGGER_PARAMS.LOG_LEVEL_DEBUG as string, // is fine in dev or uat, set this to error level in prod to limit output
