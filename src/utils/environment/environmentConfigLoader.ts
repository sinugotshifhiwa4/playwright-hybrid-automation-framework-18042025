...
import * as environmentFilePaths from "../../config/environment/environmentFilePaths";
...
export default class EnvironmentConfigLoader {
  ...
  public static async initialize(): Promise<void> {
    ...
    try {
      ...
      if (EnvironmentDetector.isRunningInCIEnvironment()) {
        logger.info(
          `CI environment detected. Skipping local environment file loading.`
        );
      ...
      await fileManager.doesDirectoryExist(
      ...
      this.initialized = true;
      logger.info(
        `Environment initialized successfully for ${env} environment`
      );
    } catch (error) {
  ...
  private static determineEnvironment(): string {
    try {
      ...
      const validEnvironments = Object.keys(
        environmentFilePaths.EnvironmentFilePaths
      );
  ...
  private static loadEnvironmentSpecificFile(fileName: string): void {
    try {
      ...
      if (fs.existsSync(filePath)) {
        ...
        logger.info(
          `Successfully loaded variables from environment file: ${baseName}`
        );
      } else {
   ...
   * @returns {void}
   */
  private static handleBaseEnvFile(
    ...
    try {
      ...
      if (baseEnvExists) {
        ...
        const shouldRequireBaseFile =
          process.env.REQUIRE_BASE_ENV_FILE === "true";
  ...
  private static setupEnvFileWatchers(): void {
    try {
      ...
      if (!fs.existsSync(envDir)) {
        logger.warn(
          `Cannot setup environment file watchers. Directory ${envDir} does not exist.`
        );
        return;
      }
